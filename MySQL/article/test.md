...
note right: 定义了 enum Functype； 设置了 type() 返回 FUNC_ITEM
...
class Item_ident{
  ...
  + void print(const THD *thd, String *str, enum_query_type query_type) const override { print(thd, str, query_type, db_name, table_name); }
...
Item <|-- Item_ident
Item_ident <|-- Item_field
Item_ident <|-- Item_ref
Item_ref <|-- Item_aggregate_ref
Item_ref <|-- Item_outer_ref
Item_ref <|-- Item_ref_null_helper
Item_ref <|-- Item_view_ref
@enduml
