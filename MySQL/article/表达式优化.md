...
4、 执行函数 FindSargablePredicates(thd, trace, &graph); // 1、遍历 graph->nodes、遍历 node.join_conditions_pushable_to_this 2、如果 graph->predicates[i].condition 可以应用索引，则把它 放到 JoinHypergraph::Node->sargable_predicates
  ...
  1.3 执行 PushDownAsMuchAsPossible 函数
             1.1.10.1 遍历条件数组，如果条件项涉及的表少于2，且 !is_join_condition_for_expr，则不会下推
